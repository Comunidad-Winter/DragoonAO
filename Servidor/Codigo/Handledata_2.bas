...
Select Case Left$(UCase$(rdata), 2)
    ...
    Case "#>" '/ULLA
        If (UserList(UserIndex).Stats.MinHP < UserList(UserIndex).Stats.MaxHP / 2) And (UserList(UserIndex).flags.Muerto = 0) Then
            Call SendData(ToIndex, UserIndex, 0, "||Nesecitas tener mas de la mitad de la vida para usar los teleports" & FONTTYPE_INFO)
            Exit Sub
        End If
        If UserList(UserIndex).flags.Paralizado = 1 Then
            ...
            Exit Sub
        End If
        If UserList(UserIndex).flags.Encarcelado = True Then
            Call SendData(ToIndex, UserIndex, 0, "||Estas Encarcelado" & FONTTYPE_INFO)
            Exit Sub
        End If
        If UserList(UserIndex).flags.DueLeanDo = 1 Then
            Call SendData(ToIndex, UserIndex, 0, "||Para salir, usa /SALIRDUELO" & FONTTYPE_INFO)
            Exit Sub
        End If
        If UserList(UserIndex).POS.Map = 197 Or UserList(UserIndex).POS.Map = 198 Or UserList(UserIndex).POS.Map = 201 Or UserList(UserIndex).POS.Map = 203 Or UserList(UserIndex).POS.Map = 204 Or UserList(UserIndex).POS.Map = 170 Or UserList(UserIndex).POS.Map = 190 Or UserList(UserIndex).POS.Map = 191 Then
            Call SendData(ToIndex, UserIndex, 0, "||No disponible en este Mapa" & FONTTYPE_INFO)
            Exit Sub
        End If
        If UserList(UserIndex).POS.Map = 201 Then
            Call SendData(ToIndex, UserIndex, 0, "||Estas entrenando. No puedes Salir de Aqui" & FONTTYPE_INFO)
            Exit Sub
        End If
    ...
    Case "#?"
        Call SendData(ToIndex, UserIndex, 0, "||Comando no Disponible en esta Version" & FONTTYPE_TALK)
        Exit Sub
    ...
    Case "#%"
        ...
        If ModoDeath = True Then
            Call SendData(ToIndex, UserIndex, 0, "||Has Sido enviado a la Zona de Deathmatch Organizada por un GM!" & FONTTYPE_WARNING)
            Call WarpUserChar(UserIndex, 199, 40, 42, True)
            Exit Sub
        End If
        Exit Sub
    ...
    Case "#+"
        ...
        If SistemaSoporte = False Then
            Call SendData(ToIndex, UserIndex, 0, "||Soporte no habilitado! Envia un Mail a soporte@dragoon-ao.com.ar" & FONTTYPE_INFO)
            Exit Sub
        End If
    ...
    Case "#-" '/DEATHMATCH
        ...
        If SistemaDeathmatch = False Then
            Call SendData(ToIndex, UserIndex, 0, "||El Deathmatch no esta Activado" & FONTTYPE_INFO)
            Exit Sub
        End If
        ...
        If Npclist(UserList(UserIndex).flags.TargetNpc).NPCtype <> NPCTYPE_DEATH Then
            Call SendData(ToIndex, UserIndex, 0, "||Primero selecciona al NPC!" & FONTTYPE_INFO)
            Exit Sub
        End If
        
        Call SendData(ToIndex, UserIndex, 0, "||Haz Ingresado al Deathmatch" & FONTTYPE_VENENO)
        Call SendData(ToAll, 0, 0, "||Sala de Deathmatch >" & UserList(UserIndex).Name & " espera contrincante en la sala de Deathmatch" & FONTTYPE_TALK)
        Call WarpUserChar(UserIndex, 202, 46, 43, True)
        Exit Sub
    ...
    Case "#~" '/INSCRIBIR
        ...
        If iPosicionTorneo = 1 Then
            Call WarpUserChar(UserIndex, 191, 56, 82, True)
            Call WarpUserChar(TIndex, 191, 54, 82, True)
            Exit Sub
        End If
        If iPosicionTorneo = 2 Then
            Call WarpUserChar(UserIndex, 191, 52, 82, True)
            Call WarpUserChar(TIndex, 191, 50, 82, True)
            Exit Sub
        End If
        If iPosicionTorneo = 3 Then
            Call WarpUserChar(UserIndex, 191, 48, 82, True)
            Call WarpUserChar(TIndex, 191, 46, 82, True)
            Exit Sub
        End If
        If iPosicionTorneo = 4 Then
            Call WarpUserChar(UserIndex, 191, 44, 82, True)
            Call WarpUserChar(TIndex, 191, 42, 82, True)
            Exit Sub
        End If
        If iPosicionTorneo = 5 Then
            Call WarpUserChar(UserIndex, 191, 40, 82, True)
            Call WarpUserChar(TIndex, 191, 38, 82, True)
            Exit Sub
        End If
        If iPosicionTorneo = 6 Then
            Call WarpUserChar(UserIndex, 191, 36, 82, True)
            Call WarpUserChar(TIndex, 191, 34, 82, True)
            Exit Sub
        End If
        If iPosicionTorneo = 7 Then
            Call WarpUserChar(UserIndex, 191, 32, 82, True)
            Call WarpUserChar(TIndex, 191, 30, 82, True)
            Exit Sub
        End If
        If iPosicionTorneo = 8 Then
            Call WarpUserChar(UserIndex, 191, 28, 82, True)
            Call WarpUserChar(TIndex, 191, 26, 82, True)
    ...
    Case "#(" '/ENTRAR
        ...
        If PosicionTorneo = 1 Then
            Call WarpUserChar(UserIndex, 191, 55, 82, True)
            Exit Sub
        End If
        If PosicionTorneo = 2 Then
            Call WarpUserChar(UserIndex, 191, 51, 82, True)
            Exit Sub
        End If
        If PosicionTorneo = 3 Then
            Call WarpUserChar(UserIndex, 191, 47, 82, True)
            Exit Sub
        End If
        If PosicionTorneo = 4 Then
            Call WarpUserChar(UserIndex, 191, 43, 82, True)
            Exit Sub
        End If
        If PosicionTorneo = 5 Then
            Call WarpUserChar(UserIndex, 191, 39, 82, True)
            Exit Sub
        End If
        If PosicionTorneo = 6 Then
            Call WarpUserChar(UserIndex, 191, 35, 82, True)
            Exit Sub
        End If
        If PosicionTorneo = 7 Then
            Call WarpUserChar(UserIndex, 191, 31, 82, True)
            Exit Sub
        End If
        If PosicionTorneo = 8 Then
            Call WarpUserChar(UserIndex, 191, 27, 82, True)
            Call SendData(ToAll, UserIndex, 0, "||Torneo Lleno" & FONTTYPE_FENIX)
            AutoTorneo = 0
            PosicionTorneo = 0
            Call SendData(ToAll, UserIndex, 0, "||Modo Torneo Desactivado." & FONTTYPE_WARNING)
